...
---------------  FOREST ARMOR + TREEVAC FUNCTIONS  ----------------------

forestUtils.queuedEvacs = {}

function forestUtils:SetForestArmorIcon(point, treevac, dir)
	if treevac then
...
function forestUtils:UnsetTerrainIcon(point)
	Board:SetTerrainIcon(point, "")
end

function forestUtils:AddQueuedEvacIfNotPresent(mechId, dir)
	if not self.queuedEvacs[mechId] then
		self.queuedEvacs[mechId] = dir
	end
end

function forestUtils:ClearQueuedEvacs()
	self.queuedEvacs = {}
end

function forestUtils.EligibleForForestArmor(pawn)
...
function forestUtils:CheckAndApplyForestArmorToSpace(p, treevac, dir)
	if self.isAForest(p) then
		self:SetForestArmorIcon(p, treevac, dir)
		return true
	else
...
function forestUtils:CheckAndApplyForestArmorToPawn(mech, treevac)
	if self.EligibleForForestArmor(mech) then
		return self:CheckAndApplyForestArmorToSpace(mech:GetSpace(), treevac, self.queuedEvacs[mech:GetId()])
	end
	return false
end

function forestUtils:RefreshForestArmorIconToAllMechs(treevac)
	local mechs = Board:GetPawns(TEAM_MECH)
	for _, mechId in pairs(extract_table(mechs)) do
		self:CheckAndApplyForestArmorToPawn(Board:GetPawn(mechId), treevac)
	end
end
